...
def sync_copa_live_state_from_extras(extras: dict) -> None:
    """Atualiza o estado da Copa 2026 usando as tabelas oficiais extraídas da FIFA."""
...
if page == "Dashboard":
    ...
    if not has_data():
        ...
        if st.session_state.equipes_copa_2026:
            section("Estado Copa 2026 detectado na FIFA", "globe")
            c_live1, c_live2 = st.columns([1, 2])
            with c_live1:
                metric_card("trophy", "Seleções da Copa", str(len(st.session_state.equipes_copa_2026)), "restrição ativa para Copa 2026")
            with c_live2:
...
# ============================================================
    ...
    with tab_net:
        st.markdown("A extração online baixa uma base histórica internacional e tenta coletar tabelas públicas adicionais da FIFA.")
        ...
        if st.button("Extrair base da internet", type="primary", width="stretch", icon=":material/cloud_download:"):
            try:
                ...
                st.success("Base histórica extraída com sucesso. O histórico será usado para treino; a Copa 2026 usará apenas as seleções detectadas nas tabelas oficiais da FIFA, quando disponíveis.")
                if st.session_state.equipes_copa_2026:
                    st.info(f"Copa 2026 ao vivo: {len(st.session_state.equipes_copa_2026)} seleções detectadas na FIFA.")
                    with st.expander("Ver seleções oficiais detectadas"):
                        st.dataframe(pd.DataFrame({"Seleção": st.session_state.equipes_copa_2026}), width="stretch", hide_index=True)
                else:
                    st.warning("Não foi possível detectar as seleções oficiais da Copa na página FIFA via HTML. O modelo ainda treina com histórico; para restringir a Copa, use uma planilha de grupos/configuração.")
...
# ============================================================
    ...
    if not has_models():
        ...
        if copa_teams:
            usar_copa = st.toggle(
                "Modo Copa 2026: usar apenas seleções classificadas/detectadas na FIFA",
                value=True,
                help="O histórico treina o modelo, mas a competição da Copa fica limitada às seleções da fase de grupos/standings da FIFA."
            )
        teams = copa_teams if usar_copa and copa_teams else all_teams

        if usar_copa and copa_teams:
            st.info(f"Modo Copa 2026 ativo: {len(copa_teams)} seleções disponíveis, vindas do estado FIFA ao vivo.")
