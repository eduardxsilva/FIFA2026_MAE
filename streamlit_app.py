...
def _padronizar_classificacao_copa_local(df: pd.DataFrame) -> pd.DataFrame:
    ...
    col_sg = _encontrar_coluna_local(d, ["SG", "GD", "Saldo", "Goal Difference"])
    col_gp = _encontrar_coluna_local(d, ["GP", "GF", "Gols Pro", "Goals For"])
    if col_eq is None:
        return pd.DataFrame(columns=["Grupo", "Posicao_Grupo", "Equipe", "Pontos", "Jogos", "SG", "GP", "Fonte_Tabela"])
    out = pd.DataFrame()
...
def carregar_estado_copa_2026_local(uploaded_file) -> dict[str, pd.DataFrame]:
    ...
    classificados = raw_classificados if isinstance(raw_classificados, pd.DataFrame) and not raw_classificados.empty else _calcular_classificados_copa_local(standings)
    if raw_update.empty:
...
# ============================================================
    ...
    tab_net, tab_file, tab_copa_local = st.tabs(["Extração da internet", "Base histórica local", "Copa 2026 local"])
            ...
            if st.button("Carregar planilha", type="primary", width="stretch", icon=":material/upload_file:"):
                ...
                try:
                    ...
                    if suffix in [".xlsx", ".xls"]:
                        try:
                            df, extras_local = carregar_excel_fifa_gerado_completo(uploaded)
                        except Exception:
                            loader = MatchDataLoader()
                    ...
                    if isinstance(metricas_tmp, pd.DataFrame) and not metricas_tmp.empty:
                        st.info(f"Métricas FIFA de equipe detectadas: {len(metricas_tmp)} linhas. Elas serão usadas no próximo treinamento.")
    ...
    with tab_copa_local:
        st.markdown(
            "Use aqui o Excel gerado pelo `baixador_fifa_local.py`. "
            "Essa planilha atualiza seleções, grupos, jogos e classificados da Copa 2026 sem depender da extração online no Streamlit Cloud."
        )
        uploaded_copa = st.file_uploader(
            "Selecione o arquivo copa2026_fifa_local.xlsx",
            type=["xlsx", "xls"],
            key="uploaded_copa_local",
        )
        if uploaded_copa is not None:
            if st.button("Carregar estado local da Copa 2026", type="primary", width="stretch", icon=":material/upload_file:"):
                try:
                    extras_local = carregar_estado_copa_2026_local(uploaded_copa)
                    extras_anteriores = st.session_state.internet_extras or {}
                    st.session_state.internet_extras = {**extras_anteriores, **extras_local}
                    st.session_state.internet_log = extras_local.get("fifa_log", pd.DataFrame()).astype(str).iloc[:, 0].tolist() if isinstance(extras_local.get("fifa_log"), pd.DataFrame) and not extras_local.get("fifa_log").empty else []
                    sync_copa_live_state_from_extras(extras_local)
                    recriar_simulador_copa_com_estado_fifa()

                    qtd_equipes = len(st.session_state.equipes_copa_2026)
                    qtd_abas_extras = len([k for k in extras_local.keys() if k.startswith("fifa_local_")])
                    st.success(f"Estado local da Copa carregado. Seleções detectadas: {qtd_equipes}. Abas extras importadas: {qtd_abas_extras}.")

                    c1, c2 = st.columns(2)
                    with c1:
                        st.write("Seleções detectadas")
                        st.dataframe(pd.DataFrame({"Seleção": st.session_state.equipes_copa_2026}), width="stretch", hide_index=True)
                    with c2:
                        st.write("Classificação/grupos")
                        if isinstance(st.session_state.df_copa2026_classificacao, pd.DataFrame) and not st.session_state.df_copa2026_classificacao.empty:
                            st.dataframe(st.session_state.df_copa2026_classificacao, width="stretch", hide_index=True)
                        else:
                            st.warning("Arquivo local sem classificação/grupos utilizáveis.")

                    jogos_local = extras_local.get("copa2026_jogos_fifa")
                    if isinstance(jogos_local, pd.DataFrame) and not jogos_local.empty:
                        with st.expander("Ver jogos importados do arquivo local"):
                            st.dataframe(jogos_local, width="stretch", hide_index=True)

                except Exception as e:
                    st.error(f"Erro ao carregar arquivo local da Copa 2026: {e}")
