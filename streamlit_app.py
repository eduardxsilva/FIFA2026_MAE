...
# ============================================================
    ...
    with tab_net:
        ...
        if st.button("🌐 Extrair base da internet", type="primary", use_container_width=True):
            try:
                ...
                st.dataframe(result.matches.head(30), use_container_width=True, hide_index=True)
    ...
    with tab_file:
        ...
        if uploaded is not None:
            if st.button("📁 Carregar planilha", type="primary", use_container_width=True):
...
# ============================================================
    ...
    if not has_data():
        ...
        if st.button("🚀 Treinar todos os modelos", type="primary", use_container_width=True):
            try:
        ...
        if has_models():
            ...
            with c1:
                st.dataframe(df_elo.head(40), use_container_width=True, hide_index=True)
            with c2:
            ...
            with st.expander("Ver forças ofensivas/defensivas"):
                st.dataframe(st.session_state.df_team_stats, use_container_width=True, hide_index=True)
    ...
    if not has_models():
        ...
        if r and r_ens:
            ...
            with c2:
                ...
                st.dataframe(pd.DataFrame(r["top_placares"]), use_container_width=True, hide_index=True)

            if r_ml:
                with st.expander("Ver previsão separada do Machine Learning"):
                    st.dataframe(pd.DataFrame([r_ml]), use_container_width=True, hide_index=True)
...
# ============================================================
    ...
    if not has_models():
        ...
        with tab_ko:
            ...
            if st.button("🏆 Simular mata-mata", type="primary", use_container_width=True):
                try:
            ...
            if st.session_state.df_simulacao_campeao is not None:
                ...
                st.plotly_chart(fig, use_container_width=True)
                st.dataframe(st.session_state.df_simulacao_campeao, use_container_width=True, hide_index=True)

        with tab_wc:
            iteracoes_copa = st.number_input("Simulações", min_value=100, max_value=20000, value=1000, step=500, key="it_wc")
            if st.button("🌍 Simular formato Copa 2026", type="primary", use_container_width=True):
                try:
            ...
            if st.session_state.df_simulacao_copa is not None:
                ...
                fig.update_layout(height=650)
                st.plotly_chart(fig, use_container_width=True)
                st.dataframe(st.session_state.df_simulacao_copa, use_container_width=True, hide_index=True)

                if st.session_state.ultima_copa_simulada is not None:
                    with st.expander("Ver última Copa simulada"):
                        st.dataframe(st.session_state.ultima_copa_simulada["classificados"], use_container_width=True, hide_index=True)
                        st.dataframe(st.session_state.ultima_copa_simulada["mata_mata"], use_container_width=True, hide_index=True)
...
# ============================================================
    ...
    if not has_data():
        ...
        if st.button("📈 Rodar validação", type="primary", use_container_width=True):
            try:
        ...
        if st.session_state.df_validacao_resumo is not None:
            ...
            st.dataframe(st.session_state.df_validacao_resumo, use_container_width=True, hide_index=True)
            ...
            with st.expander("Ver previsões testadas"):
                st.dataframe(st.session_state.df_validacao_previsoes, use_container_width=True, hide_index=True)
