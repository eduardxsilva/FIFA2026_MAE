...
class InternetDataExtractor:
    """
    Baixa uma base histórica internacional pronta para o modelo e tenta
    coletar tabelas adicionais das páginas públicas da FIFA 2026.

    A base de treino principal vem de um CSV público de resultados internacionais.
    As páginas da FIFA são salvas como dados extras quando houver tabelas HTML
    acessíveis. Sites dinâmicos podem não expor tabelas via HTML simples.
    """
    ...
    FIFA_PAGES = {
        "fifa_standings": "https://www.fifa.com/pt/tournaments/mens/worldcup/canadamexicousa2026/standings",
        "fifa_player_statistics": "https://www.fifa.com/pt/tournaments/mens/worldcup/canadamexicousa2026/statistics/player-statistics",
        "fifa_team_statistics": "https://www.fifa.com/pt/tournaments/mens/worldcup/canadamexicousa2026/statistics/team-statistics",
        "fifa_scores_fixtures": "https://www.fifa.com/pt/tournaments/mens/worldcup/canadamexicousa2026/scores-fixtures?country=BR&wtw-filter=ALL",
    }
    ...
    def extrair(self, ano_minimo: int = 2018, incluir_fifa: bool = True) -> InternetExtractionResult:
        ...
        if incluir_fifa:
            fifa_extras = self._baixar_tabelas_fifa(log)
    ...
    def _baixar_tabelas_fifa(self, log: List[str]) -> Dict[str, pd.DataFrame]:
        extras: Dict[str, pd.DataFrame] = {}

        for nome, url in self.FIFA_PAGES.items():
            try:
                log.append(f"Tentando coletar tabelas FIFA: {nome}")
                resp = requests.get(url, headers=self.headers, timeout=self.timeout)
                resp.raise_for_status()

                tabelas = pd.read_html(io.StringIO(resp.text))
                if not tabelas:
                    log.append(f"Nenhuma tabela encontrada em {nome}.")
                    continue
                ...
                for i, tabela in enumerate(tabelas, start=1):
                    ...
                    sheet_name = f"{nome}_{i}"
                    extras[sheet_name[:31]] = tabela

                log.append(f"{len(tabelas)} tabela(s) coletada(s) em {nome}.")

            except ValueError:
                log.append(f"{nome}: página acessível, mas sem tabelas HTML simples. Pode exigir Selenium.")
            except Exception as e:
                log.append(f"Falha ao coletar {nome}: {e}")
    ...
    def _montar_estado_copa_2026(self, fifa_extras: Dict[str, pd.DataFrame], log: List[str]) -> Dict[str, pd.DataFrame]:
        ...
        if standings.empty:
            log.append(
                "Copa 2026 ao vivo: não consegui extrair standings/grupos da FIFA via HTML. "
        ...
        equipes["Fonte"] = "FIFA World Cup 2026 live standings"
        ...
        log.append(f"Copa 2026 ao vivo: {len(equipes)} seleções detectadas nas tabelas FIFA.")
        if not classificados.empty:
            log.append(f"Copa 2026 ao vivo: {len(classificados)} classificados atuais calculados pela tabela.")
...
class BasePoissonModel:
    ...
    def treinar(self, df_matches: pd.DataFrame, elo_model: Optional[DynamicEloModel] = None) -> "BasePoissonModel":
        ...
        for col in ["Ataque_Casa", "Ataque_Fora", "Defesa_Casa", "Defesa_Fora", "Ataque_Geral", "Defesa_Geral", "Elo"]:
...
class MonteCarloChampionSimulator:
    ...
    def _forca_time(self, equipe: str) -> float:
        ...
        return (0.45 * ((elo - 1500) / 400)) + (0.35 * ataque) + (0.20 * (1 / max(defesa, 0.10)))
